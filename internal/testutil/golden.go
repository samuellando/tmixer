...
func CaptureStderr(f func()) string {
	return capture(&os.Stderr, f)
}

func capture(o **os.File, f func()) string {
	// Create a new read-write pipe
	r, w, _ := os.Pipe()
	// Save original out and redirect to the write end of the pipe
	old := *o
	*o = w
	...
	// Restore the original out so subsequent output goes to the real one
	*o = old
